<?php
/*
 * Documentação: Adicionar Coleção (admin/adicionar_colecao.php)
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        ...
        $sql = "INSERT INTO colecoes 
                    (nome, descricao, imagem_principal, ativa, data_registro) 
                VALUES 
                    (?, ?, ?, ?, NOW())";
        ...
        try {
            ...
            $mensagem_sucesso = "Coleção '".htmlspecialchars($nome)."' adicionada com sucesso!";
            
        } catch (PDOException $e) {
            $erros[] = "Erro ao adicionar coleção: " . $e->getMessage();
        }
    }
}
...
<head>
    ...
    <title>Roseglaze Admin - Adicionar Coleção</title>
...
<body style="background-color: #f0f0f0;">
    ...
    <main class="admin-dashboard container-produtos">
        ...
        <div class="admin-page-header">
            <h2 class="secao-titulo">Adicionar Nova Coleção</h2>
        ...
        <form action="adicionar_colecao.php" method="POST" class="form-cadastro" style="max-width: none; background-color: #fff; padding: 30px; border-radius: 8px;">
            ...
            <div class="form-grupo-minimalista">
                ...
                <input type="text" id="imagem_principal" name="imagem_principal" required>
            </div>

            <div class="form-grupo-checkbox">
                <input type="checkbox" id="ativa" name="ativa" value="1" checked> <label for="ativa">
                    Tornar esta coleção "Ativa" (visível no site)?
                </label>
            </div>

            <button type="submit" class="btn-add-to-bag">Adicionar Coleção</button>
